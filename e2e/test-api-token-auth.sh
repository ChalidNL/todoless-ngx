...
# 1. Health check
echo "--- Health check ---"
curl -sf "$HOST/api/v1/hook-health" | python3 -c "import sys,json; print(json.load(sys.stdin))"
echo ""
...
CREATE_RESP=$(curl -sf -X POST "$HOST/api/v1/api-tokens" \
...
LIST_RESP=$(curl -sf "$HOST/api/v1/api-tokens" \
  -H "Authorization: Bearer $TOKEN")
...
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$HOST/api/v1/api-tokens")
echo "Status: $STATUS"
...
curl -sf -X DELETE "$HOST/api/v1/api-tokens/$TOKEN_ID" \
